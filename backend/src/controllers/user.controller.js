...
export const addContact = async (req, res) => {
  try {
    ...
    if (user.contacts.includes(contact._id.toString())) {
      return res
        .status(409)
        .json({ message: "Contact already exists in user's contacts" });
    }
    if (user._id.toString() === contact._id.toString()) {
      return res
        .status(409)
        .json({ message: "User cannot add itself as a contact" });
    }
...
export const deleteContact = async (req, res) => {
  try {
    ...
    if (!contact) {
      return res
        .status(404)
        .json({ message: "Contact with the provided ID not found" });
    }
    if (!user.contacts.includes(contact._id.toString())) {
      return res
        .status(409)
        .json({ message: "Selected user is not a contact" });
    }
